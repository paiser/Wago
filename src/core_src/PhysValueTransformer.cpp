...
void FsTransformer::TransformOutputWrite(unsigned short* inputValue, float& transformedValue)
{
	...
	if(transformedValue != 0.0)
		tmp |= sign;
	*inputValue = tmp;
	DB_DEBUG_STREAM << "FsTransformer::TransformOutputWrite() for device *inputValue: " << *inputValue << endl;
...
{
    unsigned short N = (1 << (bitSize - 1)) - 1;
    ...
    float tmp =  CLAMP<float>((value / ((float)N /*+ 0.5*/))*boundary, -boundary, boundary);
    return tmp + MinValue;
}
...
unsigned short FsTransformer::QuantizeSigned(float value, unsigned short bitSize)
{
	 ...
	 const float sign = (value >= 0) ? 0.5 : -0.5;
