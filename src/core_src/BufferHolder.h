...
typedef struct _configInfo{
	unsigned short seriesCode; //!WAGO series number
	unsigned short couplerControllerCode; //!WAGO order number
	unsigned short firmwareVer; //!Firmware version (256 * major + minor) e.g.: V2.5 = 0x0205
	unsigned short firmwareDate[FIRMWARE_DATE_TIME_BUFFER_SIZE];//!Details of manufacture date of the firmware version,
																//max. 31 characters. If not available the value 0xFF appears.

	unsigned short firmwareTime[FIRMWARE_DATE_TIME_BUFFER_SIZE];//!Details of the manufacture time of the
																//firmware version, max. 31 characters. If
																//not available the value 0xFF appears.
	vector<short> modules;//! list of module codes.
