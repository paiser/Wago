...
void BufferHolder::RetrieveConfig(configInfo &cfg)
{
	...
	try{
		modbusInterface->ReadInputRegisters(0x2011 /*- 0x0200*/, 1, values); //Read controller series code
		cfg.seriesCode = values[0];
		values.clear();
		modbusInterface->ReadInputRegisters(0x2012 /*- 0x0200*/, 1, values);//Read controller series code
		cfg.couplerControllerCode = values[0];

		values.clear();
		modbusInterface->ReadInputRegisters(0x2010 /*- 0x0200*/, 1, values);//Read firmware version
		cfg.firmwareVer = values[0];
		values.clear();
		modbusInterface->ReadInputRegisters(0x2022 /*- 0x0200*/, FIRMWARE_DATE_TIME_BUFFER_SIZE, values);//Read firmware date
		for(unsigned int i = 0; (i < values.size()) && (i < 16); i++)
		{
			cfg.firmwareDate[i] = values[i];
		}

		values.clear();
		modbusInterface->ReadInputRegisters(0x2021 /*- 0x0200*/, FIRMWARE_DATE_TIME_BUFFER_SIZE, values); //Read firmware time
